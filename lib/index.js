...
Matcher.prototype.multiget = function(q) {
  var vComp_tmp = q.replace(/(\d)+(kg|mg|g|mcg|l|ml|\/)+/gi,"").replace(/\s{1,}/g," ").trim().split(/[\s,+\d]+/gi);
  var vComp = vComp_tmp.filter(function( item ) {
 ...
 * @returns {String} — closest match or `null`
 */
Matcher.prototype.multilist = function(q) {
  var vComp_tmp = q.replace(/(\d)+(kg|mg|g|mcg|l|ml|\/)+/gi,"").replace(/\s{1,}/g," ").trim().split(/[\s,+\d]+/gi);
  var vComp = vComp_tmp.filter(function( item ) {
    return item.length > 3 ? true : false;
  });
  ...
  for(vv in vComp){
    closest = this.list( vComp[vv] );
    var rrt = closest ? closest : null;
    ret[ vComp[vv] ] = rrt;
  }
